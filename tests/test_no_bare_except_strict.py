...
def test_no_bare_except():
    ...
    for py_path in repo_root.rglob("*.py"):
        ...
        except Exception as file_read_error:  # pragma: no cover - read error edge case
            violations.append(f"{py_path}: read error: {file_read_error}")
            continue
        for i, line in enumerate(lines):
            if BARE_EXCEPTION_PATTERN.match(line):
                violations.append(f"{py_path}:{i+1}: bare 'except Exception:' found")
    if violations:
        raise AssertionError("Bare 'except Exception:' found in files:\n" + "\n".join(violations))
